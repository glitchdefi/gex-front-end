...
import { GAS_PRICE, GAS_PRICE_GWEI } from '../../types'
...
const findGasPriceLabel = (useGas) => {
  const label = Object.keys(GAS_PRICE_GWEI).find((key) => {
    return GAS_PRICE_GWEI[key] === useGas
  });

  return label || 'custom';
}

export function useGasPriceMeta(): {
  label: string,
  gasPrice: number,
  gasPriceGwei: string,
} {
  ...
  return {
    label: findGasPriceLabel(userGas),
    gasPrice: parseInt(formatUnits(userGas, 9).toString()),
    gasPriceGwei: userGas,
  };
}
...
export function toV2LiquidityToken([tokenA, tokenB]: [Token, Token]): Token {
  return new Token(tokenA.chainId, Pair.getAddress(tokenA, tokenB), 18, 'GLCH-LP', 'GLCH LPs')
}

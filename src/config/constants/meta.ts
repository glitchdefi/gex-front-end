...
export const DEFAULT_META: PageMeta = {
  title: 'Glitch',
  description:
    'The most popular AMM on BSC by user count! Earn CAKE through yield farming or win it in the Lottery, then stake it in Syrup Pools to earn more tokens! Initial Farm Offerings (new token launch model pioneered by Glitch), NFTs, and more, on a platform you can trust.',
  image: `${process.env.NEXT_PUBLIC_GLITCH_DOMAIN_URL}/images/preview.jpg`,
}
...
export const getCustomMeta = memoize(
  (path: string, t: ContextApi['t'], _: string): PageMeta => {
    ...
    if (pathMetadata) {
      return {
        title: `${t(pathList.defaultTitleSuffix)} - ${pathMetadata.title}`,
